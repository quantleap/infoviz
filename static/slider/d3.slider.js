    ...
    selection.each(function() {
	  ...
	  minDragBehaviour.on("dragend", function() {
		  ...
		  if (navAnnual) {
			  ...
			  var barchart = d3.barchart('#row2','Yearly temperature changes', url , 'tempchangebar','yoy');
			  var comparison = d3.barchart('#row3','Average yearly temperature change over countries', '/temp_change_distribution/'.concat(low).concat('/').concat(high).concat('/').concat(currentCountryISO).concat('?bins=10&lbound=-3&ubound=3'), 'countrycomparison','cmp');
		  };	
		  if (navMonthly) {
