    ...
    selection.each(function() {
	  ...
	  minDragBehaviour.on("dragend", function() {
		  ...
		  if (navAnnual) {
			  ...
			  var comparison = d3.barchart('#row3','Country comparison', '/temp_change_distribution/'.concat(low).concat('/').concat(high).concat('/').concat(currentCountryISO).concat('?bins=10&lbound=-3&ubound=3'), 'countrycomparison','cmp');
		  };	
		  if (navMonthly) {
